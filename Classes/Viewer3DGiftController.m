...
@implementation Viewer3DGiftController
...
- (void)viewDidLoad {
	...
	[service getGiftVideoUrl:giftNumber];
	[service release];
...
-(void)initRenderingEngine
{
	if(isDismissed==NO)
	{
		NSString *giftBox3D=[giftPrefix stringByAppendingString:giftNumber];
...
-(void)aGiftWebService:(AGiftWebService*)webService ReceiveGift3DObjectArray:(NSArray*)respondData
{
	if(isDismissed==NO)
	{
		...
		NSString *objSaveName=[giftPrefix stringByAppendingString:[giftNumber stringByAppendingString:@".obj"]];
...
- (void)viewDidAppear:(BOOL)animated
{
	...
	if(modelRenderingEngine==nil)
	{
		...
		NSString *objFileName=[giftPrefix stringByAppendingString:giftNumber];
		...
		else 
		{
			...
			[service setDelegate:self];
			[appDelegate.mainOpQueue addOperation:service];
			[service ReceiveGift3DObject:giftNumber];
			[service release];
		}
	}
